...
- 変換中に `WARNING: Image contains an alpha channel. Computing a separate soft mask (/SMask) image to store transparency in PDF.` と表示される: img2pdf が透過情報付き画像(PNG など)を PDF に保存するために透過マスクを作っているだけで、処理は継続しています。出力 PDF を確認し、問題なければそのままで問題ありません。
